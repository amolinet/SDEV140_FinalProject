...
class Main_Window_Class(tkinter.Toplevel):
    ...
    def create_main_frame(self):
        self.frame = tkinter.Frame(self, bg="pink")
    ...
    def create_picture_frame(self): # used GitHub Copilot to center image and make it fill the frame
        ...
        book_icon = Image.open('george_peabody_library.jpg').resize((400, 400))
        self.book_icon_tk = ImageTk.PhotoImage(book_icon)  # Store as an instance attribute

        book_icon_label = ttk.Label(picture_frame, image=self.book_icon_tk)
    ...
    def create_book_list(self, frame):
        ...
        self.book_list_combobox.grid(row=7, column=0)
    ...
    def create_buttons(self, frame):
        insert_file_button = ttk.Button(frame, text="Add Book to .txt file", command=self.add_book_to_file) # insert is used for adding book to a text file
        insert_file_button.grid(row=4, column=3)
...
bookshelf_image_label = ttk.Label(hello_window, image=bookshelf_image_tk, background="antique white")
